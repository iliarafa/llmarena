...
- **Key Features**:
    ...
    - **LogitRunGame** (`/logit-run`): Educational minigame where users predict the most likely next token. Features two modes:
      - **Language Mode**: 30 levels across 5 categories (Idiom, Code, Movie, Fact, Logic) for linguistic prediction.
      - **Math Mode**: 30 levels across 6 categories (Arithmetic, Geometric, Fibonacci, Squares/Cubes, Constants, Binary/Hex) for mathematical pattern recognition.
      - Toggle switch to seamlessly switch between modes with auto-reset/re-shuffle.
      - Tracks score, streak, and accuracy with visual bar chart showing probability distributions.
